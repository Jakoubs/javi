...
object PersistenceModule:
  ...
  private def buildSlick(cfg: PersistenceConfig): IO[PersistenceModule] =
    SlickPersistence
      ...
      .map { slick =>
        new PersistenceModule(slick.gameDao, slick.moveEventDao, slick, slick, slick.openingDao, slick.tablebaseDao, slick.puzzleDao, slick.close())
      }
  ...
  private def buildMongo(cfg: PersistenceConfig): IO[PersistenceModule] =
    MongoPersistence
      ...
      .map { mongo =>
        ...
        new PersistenceModule(mongo.gameDao, mongo.moveEventDao, dummyUserDao, dummyFriendDao, mongo.openingDao, mongo.tablebaseDao, mongo.puzzleDao, mongo.close())
      }
