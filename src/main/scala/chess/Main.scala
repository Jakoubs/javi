...
import chess.persistence.dao.{FriendshipDao, OpeningDao, UserDao}
...
object Main:
  ...
  private def buildRestApi(): Http4sRestApi =
    try
      ...
      new Http4sRestApi(KafkaService.noOp, authService, persistence.friendshipDao, persistence.openingDao)
    catch
      case _: Throwable =>
        ...
        new Http4sRestApi(KafkaService.noOp, authService, friendshipDao, InMemoryOpeningDao)
