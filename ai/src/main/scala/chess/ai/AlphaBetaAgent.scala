...
object AlphaBetaAgent:
  ...
  private lazy val persistenceOpt =
    Try {
      ...
      error =>
        println(s"[AI][OPENING] disabled reason=persistence_unavailable error=${error.getMessage}")
        None,
      module =>
        println("[AI][OPENING] persistence=ready")
        Some(module)
    )
  ...
  private def lookupOpeningMove(state: GameState, legalMoves: List[Move]): Option[OpeningHit] =
    persistenceOpt match
      case None => None
      case Some(persistence) =>
        val fen = state.toFen
        val found =
          Try(persistence.openingDao.findBestByFen(fen).unsafeRunSync()).fold(
            error =>
              println(s"[AI][OPENING] lookup_failed fen=$fen error=${error.getMessage}")
              None,
            identity
          )
        found.flatMap { opening =>
          val parsed = CoordinateMoveParser.parse(opening.move, state).toOption
          val legal = parsed.filter(legalMoves.contains)
          if legal.isEmpty then
            println(s"[AI][OPENING] ignored_illegal fen=$fen move=${opening.move} weight=${opening.weight}")
          legal.map(m => OpeningHit(m, opening.weight))
        }
