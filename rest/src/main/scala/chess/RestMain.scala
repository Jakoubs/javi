...
object RestMain extends IOApp:
  ...
  def run(args: List[String]): IO[ExitCode] =
    ...
    val program = for {
      ...
      api         <- Resource.eval(IO(new Http4sRestApi(kafka, authService, persistence.friendshipDao, persistence.openingDao, persistence.puzzleDao, persistence.gameDao, persistence.moveEventDao)))
